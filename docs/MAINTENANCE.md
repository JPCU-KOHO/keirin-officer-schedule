...
|項目|設定|
|---|---|
|実行時刻|毎日 05:07（主実行）／05:32（予備実行、日本時間）|
|cron式|`7 20 * * *`／`32 20 * * *`（UTCの前日20:07／20:32）|
|データ取得|KEIRIN.JP選手ページとWINTICKET選手ページの予定情報|
|公開先|GitHub Pages|
|公開ブランチ|`main` の最新状態|

05:07にしている理由は、KEIRIN.JPの深夜更新後に余裕を持たせ、GitHub Actionsが混雑しやすい毎時00分を避けるためです。GitHub Actionsの定期実行はまれに遅延・取りこぼしがあり得るため、05:32にも同じ更新処理を予備実行します。主実行が成功している日も、予備実行は最新情報で上書き公開します。
