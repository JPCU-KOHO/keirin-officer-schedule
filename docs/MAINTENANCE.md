...
|項目|設定|
|---|---|
|実行時刻|毎日 03:17〜09:17（1時間ごと、日本時間）|
|cron式|`17 18 * * *`〜`17 23 * * *`（UTC前日）／`17 0 * * *`（UTC当日）|
|データ取得|KEIRIN.JP選手ページとWINTICKET選手ページの予定情報|
|公開先|GitHub Pages|
|公開ブランチ|`main` の最新状態|

KEIRIN.JPの深夜更新後に余裕を持たせ、GitHub Actionsが混雑しやすい毎時00分を避けて、03:17から09:17まで毎時実行します。各回は、公開済みJSONの基準日と対象人数を確認します。当日分（基準日が当日かつ35人以上）が公開済みなら、取得・公開を省略します。未更新または公開ページを確認できない場合だけ更新処理を試みるため、途中で失敗しても次の時刻に自動で再試行されます。

手動実行は、表示内容を更新したい場面に備えてこの確認を省略し、必ず取得・公開を行います。失敗通知Issueは、手動実行または09:17の最終試行でも更新できなかった場合だけ作成・追記します。
